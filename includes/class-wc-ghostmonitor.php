...
class WC_Ghostmonitor extends WC_Integration {
	...
	public function __construct() {
		...
		$this->method_description = __(
			'Paste your Site ID here, which can be found in your Recart <a href="https://app.ghostmonitor.com/#/plugins">account</a>. If you don’t have a Recart account sign up <a href="https://recart.com">free</a> here.',
			'woocommerce'
		);
	...
	public function init_form_fields() {
		$this->form_fields = array(
			'ghostmonitor_id'          => array(
				'title'       => __( 'Recart Account', 'woocommerce' ),
				'description' => __( 'You can find your unique Site ID on the Implementation page inside of your Recart dashboard.', 'woocommerce' ),
				'type'        => 'text',
				'default'     => ''
			),
	...
	private function collect_ghostmonitor_data_from_order( $order_id, $session_id ) {
		...
		foreach ( $order_items as $i ) {
			...
			$cart_items[] = array(
				...
				'name'       => $i['name'],
				'qty'        => $i['qty'],
				'productUrl' => get_permalink( $i['product_id'] ),
			);
	...
	private function collect_ghostmonitor_data() {
		...
		foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item_data ) {
			...
			if ( $_product && $_product->exists() && $cart_item_data['quantity'] > 0 ) {
				...
				$gm_cart_item['productUrl'] = get_permalink( $cart_item_data['product_id'] );
